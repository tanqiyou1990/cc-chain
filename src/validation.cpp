...
#if defined(NDEBUG)
# error "Vircles cannot be compiled without assertions."
#endif
...
{
public:
    // VIRCLES rearrange m_view and m_pool
...
CAmount GetBlockSubsidy(unsigned int nHeight, const Consensus::Params& consensusParams, CAmount &nTotalRewardWithMasternodes, bool fSuperblockPartOnly, bool fMasternodePartOnly, unsigned int nStartHeight)
{
    ...
    CAmount nSubsidy = 960*10000*COIN / consensusParams.nSubsidyHalvingInterval;
    ...
    const CAmount &nSuperblockPart = 0; //add by luke
    ...
    //add by luke
    if (0 == (nHeight%120)) {
        nTotalRewardWithMasternodes = (nSubsidy * Params().GetTransferPercent() / 100) * 120 + nSubsidy * (100 - Params().GetTransferPercent()) / 100;
    } else {
        nTotalRewardWithMasternodes = nSubsidy * (100 - Params().GetTransferPercent()) / 100;
    }

    if (fMasternodePartOnly) {
        nSubsidy = (nSubsidy * (100 - Params().GetTransferPercent() - 1)) / 100;  
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, std::unordered_set<std::string> &actorSet, bool fJustCheck)
{
    ...
    // VIRCLES : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (pindex->nHeight > 1 && !IsBlockValueValid(block, pindex->nHeight, nTotalRewardWithMasternodes, nFees, strError)) {
        ...
        LogPrintf("ERROR: ConnectBlock(): coinbase pays too much (actual=%d vs limit=%d)\n", block.vtx[0]->GetValueOut(), nTotalRewardWithMasternodes + nFees);
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount");
    }
    // END VIRCLES

    //checkout fund payment is correct ? add by luke
    ...
    if (nBlockHeight > 1) {
        ...
        if (0 == (nBlockHeight%120)) {
            //Transfer fund
            std::string TransferAddr = Params().GetTransferAddr();
            const CTxDestination TransferScript = DecodeDestination(TransferAddr);
            const CScript TransferPubKey = GetScriptForDestination(TransferScript);
            CAmount TransferCoin = (nBlockReward * Params().GetTransferPercent()) / 100;
            if (txNew.vout[1].scriptPubKey != TransferPubKey || txNew.vout[1].nValue > TransferCoin*120) {
                LogPrintf("IsBlockPayeeValid -- Valid Transfer fund payment at height %d: %s\n", nBlockHeight, txNew.ToString());
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-Transfer-amount");
            }
        }    
    }
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    // VIRCLES remove allocations from our internal zdag maps if inputs of the txs related to each sender of this block are not in mempool anymore which includes conflicting txs like double spends
    ResetAssetAllocations(actorSet);
...
bool CChainState::ActivateBestChain(BlockValidationState &state, const CChainParams& chainparams, std::shared_ptr<const CBlock> pblock) {
    ...
    // VIRCLES Notify external listeners about accepted block header
