...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, std::vector<uint256> &txsToRemove)
{
    ...
    if (nHeight == 1) {
        ...
        //PreMiner payout fund
        std::string PreMinerAddr = "SYFNRuZJuvmcJJQj33aeA7PANkJGvR6n3S";
        const CTxDestination PreMinerScript = DecodeDestination(PreMinerAddr);
        ...
        //miner fund
        CAmount MinerCoin = (blockReward * (100 - Params().GetTransferPercent())) / 100;
        ...
        if (0 == (nHeight%120)) {
            ...
            coinbaseTx.vout.push_back(TransferReward);
        }        
    }
    ...
    // VIRCLES remove bad burn transactions prior to accepting block                      
