...
// VIRCLES This is not a CORS check. Only check if OPTIONS are
...
static bool checkPreflight(HTTPRequest* req)
{
    ...
    // Vircles Keep origin so as not to have to set Access-Control-Allow-Origin
    // to *
    ...
    // unbounded.
    // Vircles We are only satisfying the OPTIONS request in this method in order to
    ...
    if (req->GetRequestMethod() == HTTPRequest::OPTIONS) {
        ...
        // Vircles as we are only handling OPTIONS there is
...
{    
    // VIRCLES Check if this is a preflight request - if so set the necessary
