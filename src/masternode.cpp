...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, const CPubKey& pubkey, int& nHeightRet)
{
    ...
    if(coin.out.nValue != 100 * COIN) {    //modify from 1000 to 100 by luke
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrint(BCLog::MN, "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 VCL, masternode=%s\n", outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
