    ...
    if(fMasternodeMode)
    {
        ...
        while(ifs >> pidFile){
            if(pidFile && pidFile != getpid()){
                try{
                    KillProcess(pidFile);
                    LogPrintf("%s: Virclesd successfully exited from pid %d(from virclesd.pid)\n", __func__, pidFile);
                }
                catch(...){
                    LogPrintf("%s: Virclesd failed to exit from pid %d(from virclesd.pid)\n", __func__, pidFile);
                }
            } 
        } 
        ifs.close();
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-litemode=<n>", strprintf("Disable all Vircles specific functionality (Masternodes, Governance) (0-1, default: 0)"), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-assetindex=<n>", strprintf("Index Vircles Assets for historical information (0-1, default: 0)"), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/vircles/syscoin>";
...
/** Sanity checks
 *  Ensure that Vircles is running in a usable environment with all
...
{
    // Make sure only a single Vircles process is using the data directory.
...
bool AppInitMain(NodeContext& node)
{
    ...
    // VIRCLES ********************************************************* Step 11b: Load cache data
   ...
   // force UpdatedBlockTip to initialize nCachedBlockHeight for DS, MN payments and budgets
    ...
    // ********************************************************* Step 11d: schedule Vircles-specific tasks
