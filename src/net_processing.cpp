...
static constexpr std::chrono::microseconds TX_EXPIRY_INTERVAL{std::chrono::seconds{60} * 10}; // 10 min
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // VIRCLES
    /* 
        Vircles Related Inventory Messages
...
bool static ProcessHeadersMessage(CNode *pfrom, CConnman *connman, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool via_compact_block)
{
...

    {
        ...
        if (pfrom->nVersion >= SIZE_HEADERS_LIMIT_VERSION
            ...
            maxSize = true;
        // FIXME: This change (with hasNewHeaders) is rolled back in Vircles,
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, orphan_state, porphanTx, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            ...
            if (!orphanTx.HasWitness() && orphan_state.GetResult() != TxValidationResult::TX_WITNESS_MUTATED) {
                ...
                // See https://github.com/vircles/syscoin/issues/8279 for details.
                assert(recentRejects);
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // VIRCLES Send non-tx/non-block inventory items
