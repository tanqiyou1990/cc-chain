// Copyright (c) 2013-2019 The Vircles Core developers
...
UniValue syscoinburntoassetallocation(const JSONRPCRequest& request) {
    ...
    RPCHelpMan{"syscoinburntoassetallocation",
        "\nBurns Vircles to the SYSX asset\n",
        {
...
UniValue assetallocationburn(const JSONRPCRequest& request) {
    ...
    RPCHelpMan{"assetallocationburn",
        "\nBurn an asset allocation in order to use the bridge or move back to Vircles\n",
        {
            {"asset_guid", RPCArg::Type::NUM, RPCArg::Optional::NO, "Asset guid"},
            ...
            {"ethereum_destination_address", RPCArg::Type::STR, RPCArg::Optional::NO, "The 20 byte (40 character) hex string of the ethereum destination address. Leave empty to burn to Vircles."}
        },
        RPCResult{
            "{\n"
            "  \"hex\": \"hexstring\"       (string) the unsigned transaction hexstring.\n"
...
UniValue assetallocationmint(const JSONRPCRequest& request) {
    ...
    if(ethTxRootShouldExist){
        ...
        if(nTime < txRootDB.nTimestamp) {
            throw JSONRPCError(RPC_MISC_ERROR, "Invalid Ethereum timestamp, it cannot be earlier than the Vircles median block timestamp. Please wait a few minutes and try again...");
        }
...
UniValue convertaddresswallet(const JSONRPCRequest& request)
{
    ...
    "\nConvert between Vircles 3 and Vircles 4 formats. This should only be used with addressed based on compressed private keys only. P2WPKH can be shown as P2PKH in Vircles 3. Adds to wallet as receiving address under label specified.\n",
    {
        {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The syscoin address to get the information of."},
        {"label", RPCArg::Type::STR,RPCArg::Optional::NO, "Label Vircles V4 address and store in receiving address. Set to \"\" to not add to receiving address", "An optional label"},
...
{ //  category              name                                actor (function)                argNames
   ...
   /* assets using the blockchain, coins/points/service backed tokens*/
    { "vircleswallet",            "syscoinburntoassetallocation",     &syscoinburntoassetallocation,  {"funding_address","asset_guid","amount"} }, 
    { "vircleswallet",            "convertaddresswallet",             &convertaddresswallet,          {"address","label","rescan"} },
    { "vircleswallet",            "assetallocationburn",              &assetallocationburn,           {"asset_guid","address","amount","ethereum_destination_address"} }, 
    { "vircleswallet",            "assetallocationmint",              &assetallocationmint,           {"asset_guid","address","amount","blocknumber","tx_hex","txroot_hex","txmerkleproof_hex","txmerkleproofpath_hex","receipt_hex","receiptroot_hex","receiptmerkleproof","witness"} },     
    { "vircleswallet",            "assetnew",                         &assetnew,                      {"address","symbol","description","contract","precision","total_supply","max_supply","update_flags","aux_fees","witness"}},
    { "vircleswallet",            "assetupdate",                      &assetupdate,                   {"asset_guid","description","contract","supply","update_flags","aux_fees","witness"}},
    { "vircleswallet",            "assettransfer",                    &assettransfer,                 {"asset_guid","address","witness"}},
    { "vircleswallet",            "assetsend",                        &assetsend,                     {"asset_guid","address","amount"}},
    { "vircleswallet",            "assetsendmany",                    &assetsendmany,                 {"asset_guid","inputs"}},
    { "vircleswallet",            "assetallocationlock",              &assetallocationlock,           {"asset_guid","address","txid","output_index","witness"}},
    { "vircleswallet",            "assetallocationsend",              &assetallocationsend,           {"asset_guid","address_sender","address_receiver","amount"}},
    { "vircleswallet",            "assetallocationsendmany",          &assetallocationsendmany,       {"asset_guid","address","inputs","witness"}},
    { "vircleswallet",            "sendfrom",                         &sendfrom,                      {"funding_address","address","amount"}},
};
// clang-format on
