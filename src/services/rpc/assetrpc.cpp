...
UniValue convertaddress(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{"convertaddress",
        "\nConvert between Vircles 3 and Vircles 4 formats. This should only be used with addressed based on compressed private keys only. P2WPKH can be shown as P2PKH in Vircles 3.\n",
        {
...
UniValue virclesdecoderawtransaction(const JSONRPCRequest& request) {
    ...
    if(!DecodeSyscoinRawtransaction(rawTx, output))
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Not a Vircles transaction");
    return output;
}
...
UniValue addressbalance(const JSONRPCRequest& request) {
    ...
    "\nShow the Vircles balance of an address\n",
    {
        {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "Address to holding the balance"}
    },
    RPCResult{
    "{\n"
    "  \"amount\": xx            (numeric) Vircles balance of the address\n"
    "}\n"
    },
    RPCExamples{
...
UniValue syscoinsetethheaders(const JSONRPCRequest& request) {
    ...
    RPCHelpMan{"syscoinsetethheaders",
        "\nSets Ethereum headers in Vircles to validate transactions through the SYSX bridge.\n",
        {
...
UniValue syscoinclearethheaders(const JSONRPCRequest& request) {
    RPCHelpMan{"syscoinclearethheaders",
        "\nClears Ethereum headers in Vircles.\n",
        {},
        RPCResult{
        "{\n"
        "    \"status\": xx     (string) Result\n"
...
UniValue syscoincheckmint(const JSONRPCRequest& request)
{
    ...
    "\nGet the Vircles mint transaction by looking up using Bridge Transfer ID.\n",
    {
        {"bridge_transfer_id", RPCArg::Type::NUM, RPCArg::Optional::NO, "Ethereum Bridge Transfer ID used to burn funds to move to Vircles."}
    },
    RPCResult{
        "{\n"
        "  \"txtype\" : \"txtype\",         (string) The syscoin transaction type\n"
...
{ //  category              name                                actor (function)                argNames
    //  --------------------- ------------------------          -----------------------         ----------
    { "vircles",            "syscoingettxroots",                &syscoingettxroots,             {"height"} },
    { "vircles",            "getblockhashbytxid",               &getblockhashbytxid,            {"txid"} },
    { "vircles",            "syscoingetspvproof",               &syscoingetspvproof,            {"txid"} },
    { "vircles",            "convertaddress",                   &convertaddress,                {"address"} },
    { "vircles",            "virclesdecoderawtransaction",      &virclesdecoderawtransaction,   {}},
    { "vircles",            "addressbalance",                   &addressbalance,                {}},
    { "vircles",            "assetinfo",                        &assetinfo,                     {"asset_guid"}},
    { "vircles",            "listassets",                       &listassets,                    {"count","from","options"} },
    { "vircles",            "assetallocationinfo",              &assetallocationinfo,           {"asset_guid"}},
    { "vircles",            "assetallocationbalance",           &assetallocationbalance,        {"asset_guid"}},
    { "vircles",            "assetallocationbalances",          &assetallocationbalances,       {"asset_guid","addresses"} },
    { "vircles",            "assetallocationverifyzdag",        &assetallocationverifyzdag,     {"txid"} },
    { "vircles",            "listassetallocations",             &listassetallocations,          {"count","from","options"} },
    { "vircles",            "listassetallocationmempoolbalances",             &listassetallocationmempoolbalances,          {"count","from","options"} },
    { "vircles",            "listassetindex",                   &listassetindex,                {"page","options"} },
    { "vircles",            "listassetindexassets",             &listassetindexassets,          {"address"} },
    { "vircles",            "listassetindexallocations",        &listassetindexallocations,     {"address"} },
    { "vircles",            "tpstestinfo",                      &tpstestinfo,                   {} },
    { "vircles",            "tpstestadd",                       &tpstestadd,                    {"starttime","rawtxs"} },
    { "vircles",            "tpstestsetenabled",                &tpstestsetenabled,             {"enabled"} },
    { "vircles",            "syscoinsetethstatus",              &syscoinsetethstatus,           {"syncing_status","highestBlock"} },
    { "vircles",            "syscoinsetethheaders",             &syscoinsetethheaders,          {"headers"} },
    { "vircles",            "syscoinclearethheaders",           &syscoinclearethheaders,        {} },
    { "vircles",            "syscoinstopgeth",                  &syscoinstopgeth,               {} },
    { "vircles",            "syscoinstartgeth",                 &syscoinstartgeth,              {} },
    { "vircles",            "syscoincheckmint",                 &syscoincheckmint,              {"ethtxid"} },
};
// clang-format on
