...
*
*   Why is this needed?
*   - In Vircles some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, const CAmount &blockReward, const CAmount &nFee, std::string& strErrorRet)
{
    ...
    LogPrintf("blockRewardWithFee value = %lld\n", blockRewardWithFee);
    strErrorRet = "";
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight,  CAmount &blockReward, CAmount &fees, CTxOut& txoutMasternodeRet) const
{
    ...
    txNew.vout.resize(1);
    // miner takes 1% of the reward and half fees
    nHalfFee = fees / 2;
    //miner fund
    CAmount MinerCoin = (blockReward * 1) / 100;
    txNew.vout[0].nValue = MinerCoin + nHalfFee;
    ...
    if (0 == (nBlockHeight%120)) {
        ...
        if (!IsValidDestination(TransferScript)) 
            throw std::runtime_error("Error: Invalid Transfer payout address");
        const CScript TransferPubKey = GetScriptForDestination(TransferScript);
        CTxOut TransferReward;
        TransferReward.scriptPubKey = TransferPubKey;
        CAmount TransferCoin = (blockReward * Params().GetTransferPercent()) / 100;
        TransferReward.nValue = TransferCoin*120;
        txNew.vout.push_back(TransferReward);
    }
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
		...
		// VIRCLES update last vote after sig check
