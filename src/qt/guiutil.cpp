...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Vircles address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no vircles: URI
    if(!uri.isValid() || uri.scheme() != QString("vircles"))
        return false;
...
QString formatSyscoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("vircles:%1").arg(bech_32 ? info.address.toUpper() : info.address);
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Vircles.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Vircles (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Vircles (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Vircles*.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Vircles\n";
        else
            optionFile << strprintf("Name=Vircles (%s)\n", chain);
