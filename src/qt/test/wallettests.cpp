...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<SyscoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_vircles-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_vircles-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_vircles-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = SyscoinUnits::formatWithUnit(unit, balance, false, SyscoinUnits::separatorAlways);
    ...
    // Amount input
    SyscoinAmountField* amountInput = receiveCoinsDialog.findChild<SyscoinAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: vircles:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_vircles-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
