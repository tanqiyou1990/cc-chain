...
void SyscoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // vircles: URIs or payment requests:
        if (paymentServer) {
...
{
    QMessageBox::critical(nullptr, "Runaway exception", SyscoinGUI::tr("A fatal error occurred. Vircles can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SyscoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    /// 6. Determine availability of data directory and parse vircles.conf
    ...
    // vircles: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
