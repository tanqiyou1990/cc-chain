...
const QString SYSCOIN_IPC_PREFIX("vircles:");
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the vircles: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(SYSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // vircles: URI
        {
            if (savedPaymentRequests.contains(arg)) continue;
            savedPaymentRequests.insert(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseSyscoinURI(arg, &r) && !r.address.isEmpty())
            {
    ...
    optionsModel(nullptr)
{
    ...
    // on Mac: sent when you click vircles: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start vircles: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling vircles: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("vircles://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'vircles://' is not a valid URI. Use 'vircles:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(SYSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // vircles: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseSyscoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Vircles address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
