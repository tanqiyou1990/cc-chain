...
// Copyright (c) 2017-2018 The Vircles Core developers
...
void CMasternodeMan::CheckAndRemove(CConnman& connman)
{
    ...
    
    {
        ...
        // Remove repetitive Server address ---add by luke
        std::map<COutPoint, CMasternode>::iterator it0 = mapMasternodes.begin();
        std::vector<SameNode> vSameNode;
        LogPrintf("---start vector push_back func---\n");
        while (it0 != mapMasternodes.end()) {
            SameNode samenode;
            int64_t activeTime = (int64_t)it0->second.lastPing.sigTime - it0->second.sigTime; 
            if (activeTime <= 0) {
                activeTime = (int64_t)(GetAdjustedTime() - it0->second.sigTime);
            }
            samenode.serverIp = it0->second.addr.ToString();
            samenode.payee = EncodeDestination(PKHash(it0->second.pubKeyCollateralAddress));
            samenode.activeTime = activeTime;
            vSameNode.push_back(samenode);
            ++it0;
        }
        LogPrintf("---after vector push_back func---\n");

        std::map<COutPoint, CMasternode>::iterator it1 = mapMasternodes.begin();
        while (it1 != mapMasternodes.end()) {
            CMasternodeBroadcast mnb = CMasternodeBroadcast(it1->second);
            const uint256 &hash = mnb.GetHash();
            
            int nServer = 0;
            for (std::vector<SameNode>::iterator itSN0 = vSameNode.begin(); itSN0 != vSameNode.end(); itSN0++) {
                if (itSN0->serverIp == it1->second.addr.ToString()) {
                    ++nServer;
                }
            }
            if (nServer > 1) {
                LogPrintf("---nServer=%d, serverIP=%s---\n", nServer, it1->second.addr.ToString());
                std::vector<int64_t> vActiveTime;
                int64_t bestActiveTime = 31536000;  // 1 year seconds
                for (std::vector<SameNode>::iterator itSN1 = vSameNode.begin(); itSN1 != vSameNode.end(); itSN1++) {
                    int64_t tempActiveTime;
                    if (it1->second.addr.ToString() == itSN1->serverIp) {
                        tempActiveTime = itSN1->activeTime;
                        vActiveTime.push_back(tempActiveTime);
                        if (bestActiveTime > tempActiveTime) {
                            bestActiveTime = tempActiveTime;
                        }
                    }
                }
                LogPrintf("---bestActiveTime=%lld---\n", bestActiveTime);
                for (std::vector<int64_t>::iterator itAT = vActiveTime.begin(); itAT != vActiveTime.end(); itAT++) {
                    LogPrintf("---tempActiveTime=%lld---\n", *itAT);
                    if (*itAT != bestActiveTime ) {
                        for (std::vector<SameNode>::iterator itSN2 = vSameNode.begin(); itSN2 != vSameNode.end(); itSN2++) {
                            if (*itAT == itSN2->activeTime && itSN2->payee == EncodeDestination(PKHash(it1->second.pubKeyCollateralAddress))) {
                                LogPrintf("remove payee = %s, activetime = %s, serverIP = %s\n", itSN2->payee, *itAT, itSN2->serverIp);
                                // erase all of the broadcasts we've seen from this txin, ...
                                mapSeenMasternodeBroadcast.erase(hash);
                                mWeAskedForMasternodeListEntry.erase(it1->first);

                                // and finally remove it from the list
                                it1->second.FlagGovernanceItemsAsDirty();
                                mapMasternodes.erase(it1++);
...
bool CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCountRet, masternode_info_t& mnInfoRet)
{
    ...
    */
    ...
    for (const auto& mnpair : mapMasternodes) {
        ...
        // if(fFilterSigTime && mnpair.second.sigTime + (nMnCount*4*60) > GetAdjustedTime()) continue;  //modify by luke
        int64_t WaitTime = 0;
        WaitTime = (nMnCount*4*60 < 120*60) ? nMnCount*4*60 : 120*60;
        if(fFilterSigTime && mnpair.second.sigTime + WaitTime > GetAdjustedTime()) continue;
...
{
    if(fLiteMode) return; // disable all Vircles specific functionality
