...
1. Clone the Vircles Core source code:
    ```shell
    git clone https://github.com/vpubchain/virlces
    cd virlces
    ```

2.  Build Vircles Core:

    Configure and build the headless Vircles Core binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, Vircles Core may be
...
## Running
Vircles Core is now available at `./src/virclesd`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Vircles"

touch "/Users/${USER}/Library/Application Support/Vircles/virlces.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Vircles/virlces.conf"
```

The first time you run virclesd, it will start downloading the blockchain. This process could
take many hours, or even days on slower than average systems.

You can monitor the download process by looking at the debug.log file:
```shell
tail -f $HOME/Library/Application\ Support/Vircles/debug.log
```

## Other commands:
```shell
./src/virclesd -daemon      # Starts the virlces daemon.
./src/virlces-cli --help    # Outputs a list of command-line options.
./src/virlces-cli help      # Outputs a list of RPC commands when the daemon is running.
```
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/vircles/virlces-detached-sigs).
