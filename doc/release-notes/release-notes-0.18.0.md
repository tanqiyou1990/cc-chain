...
`/Applications/Bitcoin-Qt` (on Mac) or `virclesd`/`bitcoin-qt` (on
Linux).
...
`contrib/init/virclesd.service`, it has been changed to use
`/var/lib/virclesd` as the data directory instead of
`~bitcoin/.bitcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/virclesd` will
exist has enough space (check using `df -h /var/lib/virclesd`), and
...
The systemd init file (`contrib/init/virclesd.service`) has been changed
to use `/var/lib/virclesd` as the data directory instead of
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart virclesd with the `-deprecatedrpc=generate`
...
- #14162 Also log and print messages or questions like virclesd (MarcoFalke)
