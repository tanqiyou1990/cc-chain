Vircles Core version 4.1.0.0 is now available from:

  https://github.com/vpubchain/virlces/releases/tag/v4.1.0
...
Upgrade Instructions: https://virlces.readme.io/v4.1.0/docs/virlces-41-upgrade-guide
...
BEFORE INSTALLING 4.1.0, PLEASE READ: https://virlces.readme.io/v4.1.0/docs/virlces-41-upgrade-guide
...
See: https://virlces.readme.io/v4.1.0/docs/virlces-41-upgrade-guide
...
- [Reduce memory](https://github.com/Vircles/Vircles/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Vircles Core on systems with
...
virlcescheckmint
- `virlcescheckmint` Useful for those that want to check if a Vircles Transaction
was used to mint tokens based on a Bridge Transfer ID, used by agents during the challenge
process with bridge transfer cancellations.
New settings
------------
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Vircles
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `Virclesd -help` output for these two options for
...
- `assetnew` and `assetupdate` now takes in auxfee parameters for auxfee functionality. This allows asset
  owners to define a custom fee structure to apply based on an array of transfer values, payable to an 
  address defined in the auxfee structure.
GUI changes
-----------
...
- In 4.0.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 4.1.0, this flag is now __disabled__ by default. If you want to compile Vircles Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Vircles Core 0.18.
...
- BIP 61 reject messages were deprecated in v4.0. They are now disabled
  ...
  future version of Vircles Core. (#14054)

- To eliminate well-known denial-of-service vectors in Vircles Core,
  ...
  This prevents Vircles Core from sending the BIP111 NODE_BLOOM service
  ...
  of Vircles Core and nodes that have manually enabled BIP37 support,
...
- By default, Vircles Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `Vircles-cli -getinfo` has been renamed to
...
SYSETHEREUM BRIDGE
------------------
A first-of-its-kind trust-less permission-less decentralized bridge from Vircles <-> Ethereum. Two way
bridges allow for fractional value ownership across multiple chains. You can transfer your native ERC20 to Vircles SPTs or transfer your Vircles SPT to a new ERC20 established on Ethereum. Over 2 years of research and development went into this feature so many thanks for the count-less hours nights and days spent by security researchers, developers, testers and community volunteers who helped push us create quality code that is well-tested and analyzed for secure usage.  We have spent alot of time finalizing design, re-working under security reviews and making similar assumptions of attack vectors that exist on Bitcoin today in a decentralized environment. We are confident the technology will enable value transfer across chains in a reliable, secure way and is the basis for future research on general trust-less bridge designs. The goal is to enable users benefit from utility of Vircles but in the future we will allow not only Ethereum users from benefiting from Vircless PoW security and ZDAG/Assets Payment channel functionality but more chains to come once we have generalized the solution. Read more about the bridge: https://github.com/vpubchain/sysethereum-docs

To enable to bridge, we coded 348,000 as the block height where Vircles Mint's will validate (Ethereum to Vircles transactions). Also the ERC20BalanceManager proxy contract address is defined in ChainParams so the consensus code can validate the Ethereum transaction originated on the correct contract. Geth is used to validate Ethereum transactions on Vircles so it is imperative that Geth is synced and running, it will automatically start when Vircles is launched and you may use `getblockchaininfo` to check if Geth is `synced` after a few minutes. It runs in light mode so it only needs to download roughly the last 2 weeks of block headers.

Another application that runs in the background is the [relayer](https://github.com/Vircles/relayer) which takes block information from Geth and feeds it into Vircles using `virlcessetethheaders` RPC function storing into a database. This allows Vircles core to continue operating without making inter-process calls. The functionality is re-org aware in case Ethereum re-orgs the [relayer](https://github.com/Vircles/relayer) will re-send the correct blocks, and the code in `virlcessetethheaders` will ensure the headers of Ethereum are hash-linked (current block -> prev matches previous header).

Ethereum Contracts
------------------
VirclesBattleManager - https://etherscan.io/address/0x637e110A6C61cfd3a8838eC25a2DBF75A0F34B30#code
VirclesClaimManager - https://etherscan.io/address/0x8230De6a088B455B5b83a629B0843746ED1AC5B6#code
VirclesSuperblocks - https://etherscan.io/address/0xD1e95176a81671c7aeBE68135Ba63073Aa3b642A#code
VirclesERC20Manager - https://etherscan.io/address/0x6100c8e90c4295de37f2c1114b03735193484cfd#code
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Vircles Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Vircles Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Vircles Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Vircles address without
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Vircles Core v4.0.0), this can eliminate a
...
### Bridge related
- #381 Cannot launch Geth in Rinkby mode
- #372 Precision issue on bridge
- #370 Bounty - Superblocks Future Time Bug
- #368 Bounty - CheckVirclesMint cutoffHeight attacks
- #328 virlces-geth.exe won't run if folder (windows) has space.

### Consensus
- #367 Chain Stuck on Testnet 4.1.0
- #16128 Delete error-prone CScript constructor only used with FindAndDelete (instagibbs)
- #16060 Bury bip9 deployments (jnewbery)

### Policy
- #326 Welcome screen information incorrect.
...
- #15649 Add ChaCha20Poly1305@Vircles AEAD (jonasschnelli)
...
- #15639 Vircles-wallet tool: Drop libVircles_server.a dependency (ryanofsky)
...
### RPC and other APIs
- #373 - listassetindex: "ScanAssetIndex: failed, cannot read TXIDs of asset"
...
### GUI
- #327 Sync status shows as 100% during incomplete sync 
...
- #16366 init: Use InitError for all errors in Virclesd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "Vircles-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of Virclesd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from Vircles_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_Vircles to auto generated project files (sipsorcery)
...
- #15773 Add VirclesTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_Vircles output (practicalswift)
...
- #15896 feature_filelock, interface_Vircles_cli: Use PACKAGE_NAME in messages rather than hardcoding Vircles Core (luke-jr)
...
- #16277 Suppress output in test_Vircles for expected errors (gertjaap)
...
- #16489 log: harmonize Virclesd logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (VirclesVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example Vircles.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBVircles_SERVER (Bushstar)
...
- #16080 build/doc: update Vircles_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in Vircles.conf (fanquake)
...
- #16621 add default Vircles.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the Vircles-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, Vircles.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/Virclesd.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/Vircles/Vircles/).
