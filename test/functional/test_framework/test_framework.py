...
TMPDIR_PREFIX = "vircles_func_test_"
...
class VirclesTestMetaClass(type):
    """Metaclass for VirclesTestFramework.

    Ensures that any attempt to register a subclass of `VirclesTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'VirclesTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("VirclesTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("VirclesTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class VirclesTestFramework(metaclass=VirclesTestMetaClass):
    """Base class for a vircles test script.

    Individual vircles test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def setup(self):
        ...
        self.config = config
        self.options.virclesd = os.getenv("VIRCLESD", default=config["environment"]["BUILDDIR"] + '/src/virclesd' + config["environment"]["EXEEXT"])
        self.options.virclescli = os.getenv("VIRCLESCLI", default=config["environment"]["BUILDDIR"] + '/src/vircles-cli' + config["environment"]["EXEEXT"])
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None):
        ...
        for i in range(num_nodes):
            self.nodes.append(TestNode(
                ...
                virclesd=binary[i],
                vircles_cli=self.options.virclescli,
    ...
    def _start_logging(self):
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("VirclesRPC")
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in vircles.conf
