...
# Setup the directory where our Vircles Core build for HOST will occur
...
if [ -z "$(find "${OUTDIR}/src" -name 'syscoin-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'syscoin-*.tar.gz')" "${OUTDIR}/src/"
fi

# Determine the full path to our source tarball
SOURCEDIST="$(find "${OUTDIR}/src" -name 'syscoin-*.tar.gz')"
# Determine our distribution name (e.g. syscoin-0.18.0)
...
(
    ...
    # Setup the directory where our Vircles Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Vircles Core to $INSTALLPATH
